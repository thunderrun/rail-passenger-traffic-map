...
// maps: end station path mapping, generated by graphlib dijkstra function 
...
// params
// stationCoordinates: station coordinates string e.g."123 456"
// LineCoordinates:  line coordinates string e.g."123 457"
// deviation: match deviation
// return boolean, if the coordinates matched within deviation
const matchCoordinates = ( stationCoordinates, lineCoordinates, deviation = 2) => {
  let stationCoordinatesArray = stationCoordinates.split(" ");
  let x = parseInt(stationCoordinatesArray[0]);
  let y = parseInt(stationCoordinatesArray[1]);

  let lineCoordinatesArray = lineCoordinates.split(" ");
  let lineCoordinateX = parseInt(lineCoordinatesArray[0]);
  let lineCoordinateY = parseInt(lineCoordinatesArray[1]);
  
  return (x <= (lineCoordinateX+deviation)) && (x >= (lineCoordinateX-deviation)) && (y <= (lineCoordinateY+deviation)) && (y >= (lineCoordinateY-deviation));
...
(async () => {
  ...
  // draw metro map and generate data
