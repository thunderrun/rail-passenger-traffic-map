...
(async () => {
  ...
  lines.forEach(line => {
    ...
    line.c.forEach((coordinates, index) => {
      let interval = edges.find( (edge)=> {
        return (edge.startCoordinates === coordinates || edge.endCoordinates === coordinates);
      })

      if (interval) {
        let interval2 = edges.find( (edge)=> {
          return (edge.startCoordinates === coordinates || edge.endCoordinates === coordinates) && 
            (edge.startCoordinates === startCoordinates || edge.endCoordinates === startCoordinates);
        })
        if (interval2) {
